...
frozen_parameters = [
    "dominance_map_medium_radius",
    "dominance_map_small_radius",
    "guarding_min_distance_to_shipyard",
    "guarding_max_distance_to_shipyard",
    "farming_end",
    "guarding_norm",
    "guarding_radius",
    "guarding_end",
    ...
    "max_hunting_ships_per_direction",
    "max_shipyard_distance",
    "min_shipyard_distance",
    "mining_score_farming_penalty",
...
def create_new_genome(parents, early=True):
    ...
    for characteristic in sample(hyperparameters.keys(), k=len(hyperparameters)):
        if random() <= CROSSOVER_PROBABILITY:
            possible_parents = [
                parent for parent in parents if parent != current_parent
            ]
        ...
        if random() <= MUTATION_PROBABILITY and characteristic not in (
            ...
            if hyperparameters[characteristic][0] == "float":
                genome[characteristic] = np.clip(
                    np.random.normal(
                        mutation,
                        (
                            hyperparameters[characteristic][1][1]
                            - hyperparameters[characteristic][1][0]
                        )
                        / 10,
                    ),
...
def optimize(early=True):
    ...
    for epoch in range(100000):
        ...
        for _ in range(
            min(NB_OLD_GENOMES, len(old_genomes))
        ):  # Put some random old genomes into the pool
            random_pool = load_pool(choice(old_genomes))[
                :2
            ]  # consider only the best two genomes of the pool
        ...
        if early:
            tournament = EarlyTournament(pool, 220)
        else:
            tournament = Tournament(pool)
        results = tournament.play_tournament(rounds=7)
...
def save_current_pool(pool):
    pickle.dump(
        [dict(genome) for genome in pool],
        open(
            "evolutionary/genomes/" + str(datetime.now().strftime("%Y-%m-%d %H-%M")),
            "wb",
        ),
    )
