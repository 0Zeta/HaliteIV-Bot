    ...
    def play_game(self, bots, queue):
        try:
            ...
            results = evaluate(
                "halite",
                [
                    wrap_bot(HaliteBot(bot)) if isinstance(bot, dict) else bot
                    for bot in bots
                ],
                env.configuration,
            )[0]
            ...
            for i in range(len(results)):
                if results[i] is None:
                    results[i] = -1000
                    logging.critical(
                        "An error occurred with bot "
                        + str(self.bots[self.bot_to_index(bots[i])])
                        + "."
                    )
    ...
    def play_tournament(self, rounds):
        for round in range(rounds):
            ...
            for _ in range(games_per_round):
                ...
                standings = result[1]
                new_ratings = rate(
                    [[self.ratings[self.bot_to_index(bot)]] for bot in bots],
                    ranks=standings,
                )
            ...
            print(
                [
                    (idx, rating)
                    if not isinstance(self.bots[idx], str)
                    else (
                        self.bots[idx]
                        .replace("evolutionary/bots/", "")
                        .replace(".py", ""),
                        rating,
                    )
                    for idx, rating in sorted(
                        self.ratings.items(), key=lambda item: item[1].mu, reverse=True
                    )
                ]
            )
    ...
    def get_fitness(self, step_data):
        ...
        for player in range(4):
            ...
            cargo = sum(
                [
                    ship_data[1]
                    for ship_data in step_data["observation"]["players"][player][
                        2
                    ].values()
                ]
            )
            ...
            results.append(
                halite + shipyard_count * 1200 + ship_count * 500 + cargo * 0.7
            )
        return results
